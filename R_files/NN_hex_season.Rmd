---
title: "Neural network - hexagonal grid, logged outcomes, seasonal anomalies in temperature & precipitation, single hidden layer"
...
dat.clim_all <- std.dat.m[,-grep("l.1y", names(std.dat.s))] #all seasonal std anomalies, no lagging here
...
static_features <- c("pdens", "urb", "crops", "wetlands", "medCases") #doesn't change over time #"totpop", "pop65" , , "neverCases"
...
for(i in 1:length(CZ)) {
  ...
  for (j in 1:length(pred_yrs[[i]])) {
        ...
        data_train_lag[[i]][[j]] <- forecastML::create_lagged_df(data_train[[i]][[j]],
                                                             ...
                                                             outcome_col = 1, #lookback = 1:5, #previous years used to predict
                                                             lookback_control = list(c(1:5), 0, 0, 0, 0, 0, #lag outcome 1-5, no lagging static
                                                                                     c(1:5), c(1:5), c(1:5), c(1:5), c(1:5), c(1:5), c(1:5), c(1:5), #weather lag 1-5
                                                                                     0, 0, 0, 0, 0, 0, 0, 0, #no lag current weather
                                                                                     0, 0), #nevercases, median cases already lagged
                                                             horizons = horizons,
                                                             dates = dates[[i]][[j]],
  ...
  for (j in 1:(length(pred_yrs[[i]]))) {
    ...
    # Set up forecasting dataframe
        data_forecast_lag[[i]][[j]] <- forecastML::create_lagged_df(data_train[[i]][[j]],
                                                                ...
                                                                static_features = static_features,
                                                                groups = groups)
    for(k in 1:length(data_forecast_lag[[i]][[j]])) { #need to fix current year weather, NAs from function -> SCALE!
...
saveRDS(list(data_valid=data_valid, insamp_fit=insamp_fit, insamp_fit_all=insamp_fit_all, ens_preds=ens_preds,
             outsamp_fit=outsamp_fit, cw_list=cw_list, ls_preds = ls_preds, boot_samples = boot_samples,
             model_results=model_results), "../data/NN_list_seasonal.rds")
```
