---
title: "Random Forest - hexagons (logged counts outcome, seasonal ANOMALY climate data w/o highly correlated variables)"
...
## Model details
Number of trees: 10*input variables (580);
Variables tried at each split: 1/3*input variables (19);
...
std.dat.s <- readRDS("../data/prism_temp.precip_s_std_hex.rds")

dat.clim_all <- std.dat.s[,-grep("l.1y", names(std.dat.s))] #all seasonal std anomalies, no lagging here (lags in RF setup)
...
data <- merge(data, dat.clim_all, by = c("fips","year")) %>% #WNND case info and climate info
  ...
  dplyr::select(ln_cases, location, clim.regn, year, pdens, urb:wetlands,  
                tmean.w_std:tmean.f_std, ppt.w_std:tmean.f_std_cur,
                ppt.w_std_cur:ppt.f_std_cur) #easier set up for RF where the outcome should be in column 1
```

```{r}
# Settings for RF time series
dynamic_features <- c("tmean.w_std_cur", "tmean.sp_std_cur", "tmean.su_std_cur", "tmean.f_std_cur",
...
model_function <- function(data, outcome_col=1) {
  ...
  prednames <- c(names(data)[-1])
  model_formula <- formula(paste0(outcome_names, "~", paste(prednames[prednames != "location"], 
  ...
  model <- randomForest::randomForest(formula = model_formula, data = data,
                                      ...
                                      na.action = na.omit, 
                                      keep.inbag = TRUE) #keep info on in/out of bag for calc permutation importance across multiple windows
...
for(i in 1:length(CZ)) {
  ...
  for (j in 1:length(pred_yrs[[i]])) {
    ...
    data_train_lag[[i]][[j]] <- forecastML::create_lagged_df(data_train[[i]][[j]],
                                                             ...
                                                             outcome_col = 1, 
                                                             lookback_control = list(c(1:5), 0, 0, 0, 0, 0, #lag outcome 1-5 years, no lagging static variables
                                                                                     c(1:5), c(1:5), c(1:5), c(1:5), c(1:5), c(1:5),
                                                                                     c(1:5), c(1:5), #weather lag variables 1-5 years
    ...
    model_results[[i]][[j]] <- forecastML::train_model(data_train_lag[[i]][[j]], windows[[i]][[j]], 
                                                       model_name = "Climate_division_RF", model_function, use_future = FALSE)
    ...
    data_valid[[i]][[j]] <- predict(model_results[[i]][[j]], prediction_function = list(prediction_function),
                                    data = data_train_lag[[i]][[j]]) #compared predictions to observed values from the training data
...
#              training_residuals = training_residuals), "../data/RF_s_data_int.rds")
```
...
gf_models <- lapply(1:length(model_results), function(x) { #for each climate region
    ...
    horz <- lapply(1:length(model_results[[x]][[y]]), function(z) { #for each horizon
      wind <- lapply(1:length(model_results[[x]][[y]][[z]]), function(w) { #for each window
        data.frame(rsq = tail(model_results[[x]][[y]][[z]][[w]]$model$rsq,1), #pseudo r-squared of all trees
                   ...
                   last_yr_trained = as.numeric(format(max(model_results[[x]][[y]][[z]][[w]]$date_indices), "%Y")), #last year in training data
...
imp_models <- lapply(1:length(model_results), function(x) { #for each climate division
...
## Forecast models using trained RFs
...
for(i in 1:length(CZ)) {
  ...
  for (j in 1:length(pred_yrs[[i]])) {
    ...
    # Bootstrap and pull SDs from samples
    set.seed(100)
    ...
    if(all(!is.na(forecasts[[i]][[j]]$ln_cases_pred))) { #predictions for all horizons
      ...
      boot_samples[[i]][[j]] <-intervals[[i]][[j]][["samples"]] %>% #pulls only the samples
        dplyr::select(ln_cases_pred) %>% #ln_case_pred
        rename(sampled_pred = ln_cases_pred) %>% #ln_cases_pred
        tidyr::drop_na()
      ...
      boot_pred_sd[[i]][[j]] <- forecasts[[i]][[j]] %>%
        ...
        group_by(model_forecast_horizon, horizon, forecast_period, location) %>% #finds SD of the samples
         summarize(q50 = quantile(sampled_pred, 0.5), #median across samples
                  ...
                  q50u = quantile(sampled_pred, 0.75)) %>%
      ...
      intervals[[i]][[j]] <-forecastML::calculate_intervals(forecasts[[i]][[j]][!is.na(forecasts[[i]][[j]]$ln_cases_pred),], #predictions, non-NA #ln_cases_pred
                                                            training_residuals[[i]][[j]]$residuals, #residuals from the training data
                                                            keep_samples = TRUE, times = boot_num) #Keep_samples to look at distribution of preds
      
      boot_samples[[i]][[j]] <- intervals[[i]][[j]][["samples"]] %>% #pulls only the samples
        dplyr::select(ln_cases_pred) %>% #ln_cases_pred
        rename(sampled_pred = ln_cases_pred) %>% #ln_cases_pred
        tidyr::drop_na()
      ...
      boot_pred_sd[[i]][[j]] <- forecasts[[i]][[j]][!is.na(forecasts[[i]][[j]]$ln_cases_pred),] %>% #ln_cases_pred
        slice(rep(row_number(), boot_num)) %>%
        bind_cols(., boot_samples[[i]][[j]]) %>% #merges bootstrapped samples + forecasts
        mutate(sampled_pred = pmax(0, sampled_pred)) %>% #nonsensical neg cases to 0
        group_by(model_forecast_horizon, horizon, forecast_period, location) %>% #finds SD of the samples
      ...
      boot_cases[[i]][[j]] <- forecasts[[i]][[j]][!is.na(forecasts[[i]][[j]]$ln_cases_pred),] %>% #ln_cases_pred
        slice(rep(row_number(), boot_num)) %>%
        bind_cols(., boot_samples[[i]][[j]]) %>% #merges bootstrapped samples + forecasts
        mutate(case_pred = pmax(0, round(exp(sampled_pred)-1))) %>% #convert pred to # cases (integer, 0 min)
        select(-ln_cases_pred)
...
mse_preds <- lapply(seq_along(boot_cases), function(i) {
  ...
  left_join(bc, ln_cases_country[[i]][ln_cases_country[[i]]$year > 2014, c("ln_cases","location","year")] %>% 
    ...
    summarise(rmse = sqrt(mean((sampled_pred - ln_cases)^2)),#calc RMSE for prediction
...
saveRDS(ls_preds, "../data/ls_preds_seasonal.rds") #to compare with all other models (used in comparison_models_hex.R script)


mls <- lapply(ls_preds, function(x) {
  x %>% group_by(location, horizon, clim.regn) %>%
    summarise(mscore_nb = mean(logscore_nb)) %>% #mean logscore for each location's model (multiple windows) at each horizon
